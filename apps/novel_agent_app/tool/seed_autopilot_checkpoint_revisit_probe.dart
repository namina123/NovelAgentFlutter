...
Future<void> main() async {
  // 中文注释: 该探针验证 mode 1 的高风险 checkpoint 能回收到真正可读的长期约束回看包。
  final apiConfig = await loadProbeApiConfig();
  final provider = ProviderEndpointSettings(
    id: 'seed_checkpoint_revisit_probe',
    title: 'Seed Checkpoint Revisit Probe',
    protocol: 'openai_compatible',
    baseUrl: apiConfig.baseUrl,
    apiKey: apiConfig.apiKey,
    modelId: apiConfig.modelId,
    description: 'Long task checkpoint revisit probe',
  );
  ...
  final projectRoot = await Directory.systemTemp.createTemp(
    'novel_agent_seed_checkpoint_revisit_probe_',
  );
  ...
  try {
    ...
    await taskRepository.saveRecord(
      ...
      <String, Object?>{
        ...
        'severity': 'high',
        'severity_label': '高风险',
        'severity_reasons': <Object?>['探针覆盖长期约束回看动作。'],
        'persistent_context_paths': <Object?>[
          'tracking/modes/seed_autopilot_novel/guidance.md',
          'styles/seed_autopilot_style.md',
        ],
    ...
    final revisit = await workflowRuntimeService.applyCheckpointReviewAction(
      project,
      checkpointReviewPath,
    ...
    report.addAll(<String, Object?>{
      'ok':
          ...
          ValueReaders.boolValue(revisit['ok']) &&
          ValueReaders.stringList(revisit['focus_domains']).isNotEmpty &&
          ValueReaders.mapList(revisit['items']).any(
            (item) =>
      ...
      'revisit_enabled': ValueReaders.mapList(actionPackage['actions']).any(
        (item) =>
            ValueReaders.stringValue(item['id']) == 'revisit_mode_guidance' &&
            ValueReaders.boolValue(item['enabled']),
      ),
      'revisit_ok': ValueReaders.boolValue(revisit['ok']),
      'revisit_error': ValueReaders.stringValue(revisit['error']),
      'focus_domains': ValueReaders.stringList(revisit['focus_domains']),
      'recommended_paths': ValueReaders.stringList(
        revisit['recommended_paths'],
      ),
    });
  } finally {
    final reportPath = File(
      '${Directory.current.path}${Platform.pathSeparator}artifacts${Platform.pathSeparator}seed_autopilot_checkpoint_revisit_probe_report.json',
    );
    ...
    stdout.writeln(
      ValueReaders.boolValue(report['ok'])
          ? 'seed_autopilot_checkpoint_revisit_probe: PASS'
          : 'seed_autopilot_checkpoint_revisit_probe: FAIL',
    );
