...
Future<void> main() async {
  // 中文注释: 该探针验证 mode 1 的返工链在 revision 后处理后，能真实回写 postprocess 报告与 checkpoint 复盘。
  final apiConfig = await loadProbeApiConfig();
  final provider = ProviderEndpointSettings(
    id: 'seed_revision_postprocess_probe',
    title: 'Seed Revision Postprocess Probe',
    protocol: 'openai_compatible',
    baseUrl: apiConfig.baseUrl,
    apiKey: apiConfig.apiKey,
    modelId: apiConfig.modelId,
    description: 'Long task revision postprocess probe',
  );
  ...
  final projectRoot = await Directory.systemTemp.createTemp(
    'novel_agent_seed_revision_postprocess_probe_',
  );
...
''',
    ...
    final execution = await workflowRuntimeService.loadWorkflowTaskExecution(
      project,
      revisionSelector,
    );
    final postprocessCheckpointReview = ValueReaders.mapValue(
      postprocess['checkpoint_review'],
    );
    report.addAll(<String, Object?>{
      'ok':
          ...
          ValueReaders.stringValue(
            reloadedTask['postprocess_review_report_path'],
          ).startsWith('reviews/') &&
          ValueReaders.stringValue(
            reloadedTask['postprocess_checkpoint_review_path'],
          ).startsWith('tracking/checkpoint_reviews/') &&
          ValueReaders.stringList(
            execution['postprocess_output_paths'],
          ).isNotEmpty &&
          ValueReaders.stringValue(revisionTask['mode']) ==
      ...
      'postprocess_changed_paths': ValueReaders.stringList(
        postprocess['changed_paths'],
      ),
      'checkpoint_review_path': ValueReaders.stringValue(
        postprocessCheckpointReview['relative_path'],
      ),
      'revision_mode': ValueReaders.stringValue(revisionTask['mode']),
    });
  } finally {
    final reportPath = File(
      '${Directory.current.path}${Platform.pathSeparator}artifacts${Platform.pathSeparator}seed_autopilot_revision_postprocess_probe_report.json',
    );
    ...
    stdout.writeln(
      ValueReaders.boolValue(report['ok'])
          ? 'seed_autopilot_revision_postprocess_probe: PASS'
          : 'seed_autopilot_revision_postprocess_probe: FAIL',
    );
