...
Future<void> main() async {
  ...
  final provider = ProviderEndpointSettings(
    ...
    title: 'Seed Checkpoint Action Probe',
    protocol: 'openai_compatible',
    baseUrl: apiConfig.baseUrl,
    apiKey: apiConfig.apiKey,
    modelId: apiConfig.modelId,
    description: 'Long task checkpoint action probe',
  );
  ...
  final projectRoot = await Directory.systemTemp.createTemp(
    'novel_agent_seed_checkpoint_action_probe_',
  );
  ...
  try {
    ...
    report.addAll(<String, Object?>{
      'ok':
          ...
          ValueReaders.boolValue(actionPackage['ok']) &&
          ValueReaders.stringValue(actionPackage['severity']).isNotEmpty &&
    ...
    final reportPath = File(
      '${Directory.current.path}${Platform.pathSeparator}artifacts${Platform.pathSeparator}seed_autopilot_checkpoint_action_probe_report.json',
    );
    ...
    stdout.writeln(
      ValueReaders.boolValue(report['ok'])
          ? 'seed_autopilot_checkpoint_action_probe: PASS'
          : 'seed_autopilot_checkpoint_action_probe: FAIL',
    );
