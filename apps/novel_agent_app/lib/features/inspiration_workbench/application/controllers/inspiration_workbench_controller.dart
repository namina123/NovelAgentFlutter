...
import '../../../workbench/application/services/workbench_opening_launch_bridge_service.dart';
...
class InspirationWorkbenchController extends ChangeNotifier
  ...
  Future<void> onInspirationWorkbenchLongTaskLaunchRequested() async {
    // 中文注释: 长任务启动只消费已经投影好的入口状态，控制器不再自行根据项目类型重判定。
    final launchViewData = _viewData.longTaskLaunch;
    if (!launchViewData.isVisible || !launchViewData.canLaunch) {
      _statusMessage = launchViewData.isVisible
          ? '当前灵感阶段还未收束完成，暂时不能启动长任务。'
          : '当前项目不提供长任务启动入口。';
      _rebuildView();
      return;
    }
    ...
    try {
      ...
      if (ValueReaders.boolValue(result['ok'], true)) {
        _statusMessage =
            '${ValueReaders.stringValue(result['message'], '长任务队列已生成。')} 已自动切到长任务总站。';
        _snapshot = _snapshot.copyWith(isLoading: false);
        _rebuildView();
        await _showTaskCenterRequested();
        return;
      }
      _statusMessage = ValueReaders.stringValue(result['message'], '启动长任务失败。');
      _snapshot = _snapshot.copyWith(isLoading: false);
      _rebuildView();
    } catch (error) {
      _snapshot = _snapshot.copyWith(isLoading: false);
