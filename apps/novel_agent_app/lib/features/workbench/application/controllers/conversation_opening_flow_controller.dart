...
class ConversationOpeningFlowController {
  ...
  Future<void> onPrimaryActionRequested(String actionId) async {
    ...
    if (action == null) {
      // 中文注释: 主动作不在当前 primaryActions 里（多半是项目还没到对应就绪态，如续写需要正文基础）。
      // 如实点名 + 给就绪提示，避免静默 _announce 被误读成"活动开始"而让上层无限等待（曾导致续写 2 分钟超时）。
      _controller._announce(
        '当前没有可用的“$actionId”入口。项目可能仍在开局阶段，'
        '请先用开局收束方向或导入/拆书补齐正文基础，达到续写就绪后再继续。',
      );
      return;
    }
