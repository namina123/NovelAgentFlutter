...
class ProviderConnectionValidationService {
  ...
  ProviderConnectionValidationResult validate({
    ...
    // 中文注释: 连接验证要统一产出可消费合同，避免 widget 或 view-data 自己判断协议、路由和回退规则。
    ...
    final runtimeRoute = _runtimeRouteContract ??
        ProviderRuntimeRouteContract.resolve(
          protocolKind: protocolKind ?? connectionContract.protocolKind ?? ProtocolKind.openAiCompatible,
    ...
    if (protocolKind != null && connectionContract.protocolKind != null &&
        protocolKind != connectionContract.protocolKind) {
      errors.add('当前协议与接口模板不一致。');
      hideOptions.addAll(connectionContract.allowedApiModes.where(
    ...
    return ProviderConnectionValidationResult(
      ...
      matchedTemplateId: ValueReaders.stringValue(contractResolution.matchedTemplate['id']),
