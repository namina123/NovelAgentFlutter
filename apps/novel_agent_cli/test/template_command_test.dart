...
import 'package:novel_agent_cli/commands/template/template_command.dart';
      ...
      expect(
        ...
        contains(
          predicate<_PrintedBlock>(
            (block) =>
                block.title == 'template help' &&
                block.content.contains('template list [--project 路径]') &&
                block.content.contains('template restore --id review_report'),
          ),
        ),
      );
    });

    test('save formats output path through shared artifact label service', () async {
      final bundle = _buildCommand(
...
({TemplateCommand command, _RecordingTerminalPrinter printer}) _buildCommand({
