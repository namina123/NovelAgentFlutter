...
class SkillGroupValidatorService {
  JsonMap validate(JsonMap group) {
    ...
    final skills = ValueReaders.stringList(group['skills']);
    if (id.isEmpty) {
      errors.add('技能组缺少 id。');
    }
    if (name.isEmpty) {
      errors.add('技能组缺少 name。');
    }
    if (description.isEmpty) {
      errors.add('技能组缺少 description。');
    }
    if (version.isEmpty) {
      errors.add('技能组缺少 version。');
    }
    if (skills.isEmpty) {
      warnings.add('建议至少声明一个 skill，避免空技能组进入 proposal 流程。');
    }
