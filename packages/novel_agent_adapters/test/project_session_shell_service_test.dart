...
void main() {
  group('ProjectSessionShellService', () {
    test('lists, loads and stats sessions from the shared workspace contract', () async {
    ...
    test('send compact and stop all persist back into the same session store', () async {
      ...
      await sessionWorkspaceService.saveSession(
        project,
        <String, Object?>{
          'id': 'session_send',
          'title': '发送会话',
      ...
      final stopResult = await shellService.stopSession(project, 'session_send');
      ...
      expect(
        ValueReaders.boolValue(
          ValueReaders.mapValue(sendResult['compaction_decision'])['should_compact'],
        ),
        isTrue,
      );
      expect(
        ValueReaders.objectList(
          ValueReaders.mapValue(sendResult['session_prompt_context'])['history_messages'],
        ),
        isNotEmpty,
      );
      ...
      expect(
        ...
        lessThan(ValueReaders.intValue(compactResult['transcript_message_count'])),
      );
      expect(ValueReaders.boolValue(stopResult['ok']), isTrue);
      expect(ValueReaders.stringValue(stopResult['workflow_stage']), 'stopped');
      expect(ValueReaders.stringValue(stopResult['public_status']), '已停止');
      expect(ValueReaders.stringValue(storedAfterStop['public_status']), '已停止');
    });

    test('resume reopens the active stopped session and returns the shared prompt context', () async {
      ...
      expect(ValueReaders.stringValue(resumeResult['resume_source']), 'active_session');
      expect(ValueReaders.stringValue(resumeResult['workflow_stage']), 'draft');
      expect(ValueReaders.stringValue(resumeResult['public_status']), '创作已启动');
      expect(
        ValueReaders.stringValue(resumeResult['context_markdown']),
        contains('继续写作的起点'),
      );
      expect(ValueReaders.stringValue(storedAfterResume['workflow_stage']), 'draft');
      expect(ValueReaders.stringValue(storedAfterResume['public_status']), '创作已启动');
