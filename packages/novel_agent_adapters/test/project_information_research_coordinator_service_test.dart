...
void main() {
  group('ProjectInformationResearchCoordinatorService', () {
    ...
    test(
      'approved network request can execute gateway even under safe host',
      () async {
    ...
    test(
      'import request saves imported research note and completes request',
      () async {
        ...
        expect(ValueReaders.stringValue(record['request_state']), 'completed');
        expect(
          ValueReaders.boolValue(record['import_execution_performed']),
          isTrue,
        );
        expect(
    ...
    test(
      'permission blocked request returns structured blocked state',
      () async {
        await workspacePort.writeTextFile(
          project.rootPath,
          'research/blocked.md',
