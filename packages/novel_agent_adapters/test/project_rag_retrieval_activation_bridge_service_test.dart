...
void main() {
  group('ProjectRagRetrievalActivationBridgeService', () {
    test('builds a formal retrieval activation package from raw retrieval result', () {
      ...
      final activation = bridge.buildPackage(
        project,
        <String, Object?>{
      ...
      expect(activation.activationPackageId, 'rag_activation:project-rag-activation-1:query-001');
      expect(activation.querySummary, contains('project=project-rag-activation-1'));
      expect(activation.querySummary, contains('query=镜潮回扣'));
      expect(activation.selectedHits, hasLength(1));
      expect(activation.selectedHits.single.evidencePath, 'chapters/01.md#L0-L16');
    ...
    test('falls back to hit-derived summaries and warnings when raw summaries are absent', () {
      const bridge = ProjectRagRetrievalActivationBridgeService();
      final project = ProjectDescriptor(
