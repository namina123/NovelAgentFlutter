...
resource_hints:
  ...
  references:
    ...
    - references/style-modules/index.md
  assets:
    - assets/codex-continue-novel.ps1
preferred_output: 面向长篇小说的项目控制、章节控制与持续续写结果
---
...
## NovelAgent Compatibility

This builtin skill is adapted from an MIT-licensed upstream package and must obey the current NovelAgent architecture instead of introducing a competing project schema.
...
- treat `assets/codex-continue-novel.ps1` as an optional desktop handoff asset rather than a mandatory project file
- keep all project truth in the active NovelAgent workspace; this skill may guide the workflow, but it must not become a hidden second source of truth
...
Inside NovelAgent, these are logical control surfaces first. Prefer mapping them into the active workspace structure:

- `00-project-overview.md` + `01-theme-and-proposition.md` -> `specs/`
- `02-worldbuilding.md` + `06-foreshadow-ledger.md` -> `world/`
- `03-cast-bible.md` + `04-relationship-map.md` -> `characters/`
- `05-main-plotlines.md` + `07-chapter-roadmap.md` -> `outline/`, `volume_outlines/`, `chapter_outlines/`
- `08-dynamic-state.md` -> `summaries/` or mode/task state files chosen by the current strategy
- `09-style-guide.md` -> `styles/`
- `control-cards/` -> strategy-specific task or control-card directories when the current mode already defines them
- `logs/writing-log.md` -> `tracking/`, `runs/`, or other audit outputs produced by the active runtime
...
Inside NovelAgent, this bootstrap script is optional and desktop-only:

- do not create it on Android or iOS
- do not require it for GUI-first project startup
- only materialize it when the current workflow explicitly needs a PowerShell handoff or marathon continuation entry on desktop Windows
