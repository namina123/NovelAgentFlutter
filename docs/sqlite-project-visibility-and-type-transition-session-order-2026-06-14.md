...
- `SPT-08`：
- `SPT-09`：
- `SPT-10`：2026-06-15 已完成。关键修改点：确认项目类型与存储载体的能力矩阵已由 `ProjectTypeDefinition`、`ProjectTypeCatalogService`、`ProjectStorageStrategy`、`ProjectManifestCodecService`、`ProjectTypeTransitionPolicy`、`ProjectTypeTransitionPreparationService`、`ExecuteProjectTypeTransitionUseCase`、`CreateProjectWorkspaceUseCase` 与 `ProjectStorageAwareToolCapabilityMatrix` 共同收口；`knowledge_base` 在 core 层明确收束为 `sqlite_project_store` only，`novel <-> long_novel` 第一阶段互转图被固定为唯一可用图，且互转执行会保留原存储策略不变，只补运行基准和运行配置，不把类型转换和存储迁移混成一件事。`SqliteVisibilityPolicy` 进一步把 SQLite 项目的语义树和可见性边界写成正式合同。主要文件：`packages/novel_agent_core/lib/src/project/project_type_definition.dart`、`packages/novel_agent_core/lib/src/project/project_type_catalog_service.dart`、`packages/novel_agent_core/lib/src/project/project_storage_strategy.dart`、`packages/novel_agent_core/lib/src/project/project_type_transition_policy.dart`、`packages/novel_agent_core/lib/src/project/project_type_transition_preparation_service.dart`、`packages/novel_agent_core/lib/src/use_cases/execute_project_type_transition_use_case.dart`、`packages/novel_agent_core/lib/src/project/project_manifest_codec_service.dart`、`packages/novel_agent_core/lib/src/project/sqlite_visibility_policy.dart`、`packages/novel_agent_core/lib/src/project/project_storage_aware_tool_capability_matrix.dart`、`packages/novel_agent_core/lib/src/use_cases/create_project_workspace_use_case.dart`。测试/验证结果：`dart test test/project_type_catalog_service_test.dart test/project_type_transition_policy_test.dart test/project_manifest_storage_strategy_test.dart test/project_type_transition_use_case_test.dart test/create_project_workspace_use_case_test.dart test/project_storage_aware_tool_capability_matrix_test.dart test/sqlite_projection_visibility_contracts_test.dart` 通过。残留风险：后续 `SPT-12 ~ SPT-16` 仍需把这些合同持续下沉到 GUI / ViewModel / probe 消费层，避免在壳层重新长出第二套项目类型语义。
- `SPT-11`：2026-06-15 已完成。关键修改点：确认 `knowledge_base` 与参考资产库的对齐桥已由 `SqliteReferenceEvidenceSubstrate`、`SqliteProjectReferenceAttachmentLayer`、`ProjectReferenceProjectionService`、`ProjectReferenceAlignmentSummaryService` 与 `ProjectReferenceExtractionExecutionService` 共同承载；项目资料、项目资料挂载、参考资产库三层术语被固定到对齐摘要与投影 metadata 中，`knowledge_base` 不再在产品面上被误导成“项目知识卡合集”。`ProjectReferenceAlignmentSummaryService` 负责输出 `knowledge/项目资料总览.md` 这类项目资料总览，明确区分项目私有草稿资产、待审核资产、可提升到参考资产库的正式资产。主要文件：`packages/novel_agent_adapters/lib/src/storage/sqlite_reference_evidence_substrate.dart`、`packages/novel_agent_adapters/lib/src/storage/sqlite_project_reference_attachment_layer.dart`、`packages/novel_agent_adapters/lib/src/storage/project_reference_projection_service.dart`、`packages/novel_agent_adapters/lib/src/storage/project_reference_alignment_summary_service.dart`、`apps/novel_agent_app/lib/features/project_assets/application/services/project_reference_extraction_execution_service.dart`。测试/验证结果：`dart test test/project_reference_alignment_summary_test.dart test/sqlite_reference_evidence_substrate_test.dart` 通过；`flutter test test/project_reference_extraction_execution_service_test.dart` 通过。残留风险：`SPT-12 ~ SPT-16` 还要继续把这些术语在 GUI 创建链、工作台与 probe 中消费掉，避免壳层重新出现“项目知识卡合集”的旧说法。
- `SPT-12`：2026-06-15 已完成。关键修改点：确认 GUI 创建链已由 `ProjectLauncherViewDataService`、`ProjectCreatePanel`、`ProjectStorageStrategyOptionTile` 与 `app_shell_controller` 的创建/保存入口共同消费新的能力矩阵；`knowledge_base` 在创建面板中只展示 SQLite 选项，且文案已经明确指向“资料知识库 / 参考资产治理”，默认标题也收束为“未命名资料知识库”，没有再把它包装成普通写作壳。普通项目 `novel / long_novel / book_deconstruction` 仍保留合法的 Markdown 与 SQLite 选项。主要文件：`apps/novel_agent_app/lib/features/workbench/application/services/project_launcher_view_data_service.dart`、`apps/novel_agent_app/lib/features/workbench/presentation/widgets/project_create_panel.dart`、`apps/novel_agent_app/lib/features/workbench/presentation/widgets/project_storage_strategy_option_tile.dart`、`apps/novel_agent_app/lib/app/state/app_shell_controller.dart`。测试/验证结果：`flutter test test/project_launcher_view_data_service_test.dart` 通过。残留风险：后续 `SPT-13 ~ SPT-14` 还要继续把这些矩阵消费到项目设置互转入口和 SQLite 工作台语义树中，避免旧语义从其它入口回流。
- `SPT-13`：2026-06-15 已完成。关键修改点：确认项目设置中的类型互转入口已由 `WorkbenchProjectPanelActionPolicyService`、`ProjectTypeTransitionWorkspaceCommandViewDataService`、`WorkbenchWorkspaceController` 与 `app_shell_controller` 共用 core 的 `ProjectTypeTransitionPlan` 和 `ProjectTypeTransitionPreparationService`；GUI 只在 `novel / long_novel` 上暴露“项目类型转换”动作，并把拒绝原因、目标类型选项与运行基准选项统一从 core 计划投影出来，不再自己判断能否转换。知识库未开放互转，且入口层描述仍保持“存储策略保持不变”的正式边界。主要文件：`apps/novel_agent_app/lib/features/workbench/application/services/workbench_project_panel_action_policy_service.dart`、`apps/novel_agent_app/lib/features/workbench/application/services/project_type_transition_workspace_command_view_data_service.dart`、`apps/novel_agent_app/lib/features/workbench/application/controllers/workbench_workspace_controller.dart`、`apps/novel_agent_app/lib/app/state/app_shell_controller.dart`。测试/验证结果：`flutter test test/workbench_project_panel_action_policy_service_test.dart test/project_type_transition_workspace_command_view_data_service_test.dart` 通过。残留风险：后续 `SPT-14` 还要继续把 SQLite 项目的语义树和渲染边界收束好，避免入口开放了但工作台展示仍旧偏 file-tree first。
- `SPT-14`：
